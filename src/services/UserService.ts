...
export default class UserService {
    ...
    static async getUsers(component: ComponentPublicInstance, id: number) {
        try {
    ...
    static async getUser(component: ComponentPublicInstance) {
        try {
            const response = await component.axios.get(ConstantTool.BASE_URL + '/user', {
                headers: { Authorization: useAppStore().session.token, },
            })
    ...
    static async postUser(component: ComponentPublicInstance, payload: User) {
        try {
